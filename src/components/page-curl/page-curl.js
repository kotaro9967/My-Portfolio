...
const DEFAULTS = {
  scrollDistance: 3200, // この px 分ホイール／スワイプすると完全にめくり切る（さらに重く・かなりしっかりスクロールしないと開かない）
  smoothing: 0.11,      // 0-1。大きいほどスクロールへの追従が速い（小さくして紙の重さ・引っかかり感を強調）
  flickThreshold: 150,  // 一回の入力量がこれ以上なら「強いスクロール」とみなして自動で最後までめくる（普通のホイール1クリックでは開かない強さ）
  settleDelay: 450,     // 入力が止まってからこの時間、中途半端なら元の状態へ戻す (ms)。
                         // scrollDistance が大きいほど「ホイールをノッチごとに回す」ような断続的な入力でも
                         // 各ノッチの間隔より確実に長くする必要がある（短すぎると本気で回し続けていても
                         // ノッチの合間に押し戻されて絶対に開ききれなくなる）
